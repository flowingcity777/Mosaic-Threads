...
document.addEventListener("DOMContentLoaded", () => {
  ...
  paletteButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      showMessage(`Selected ${btn.textContent.trim()} palette.`, "info");
    });
  });
});
...
function generatePattern() {
  ...
  if (gridWidth < 10 || gridWidth > 120) {
    showMessage("Pattern width should be between 10 and 120.", "error");
    return;
  }
  ...
  pattern = smoothGrid(pattern, palette);
...
function smoothGrid(grid, palette) {
  ...
  return grid.map((row, y) =>
    row.map((cell, x) => {
      ...
      const mostCommonName = Object.entries(counts).sort((a, b) => b[1] - a[1])[0][0];
      return palette.find((c) => c.name === mostCommonName) || cell;
    })
  );
}
      ...
      html += `
        <div
          ...
          style="
            ...
            width:${cellSize}px;
            height:${cellSize}px;
            background-color:rgb(${cell.rgb[0]}, ${cell.rgb[1]}, ${cell.rgb[2]});
...
function renderInstructions(grid, stitchesPerInch, yarnWeight, palette) {
  ...
  instructionsDiv.innerHTML = `
    ...
    <p style="font-family:monospace;font-size:12px;line-height:1.5;">
...
function generateRowInstructions(grid) {
  return grid
    .map((row, i) => {
      const sequence = row.map((cell) => cell.name[0]).join(" ");
...
function downloadPattern() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      ctx.fillText(cell.name[0], px + cellSize / 2, py + cellSize / 2);
    }
  }

  const link = document.createElement("a");
  link.download = "mosaic-threads-pattern.png";
